...
get_data <-function(db = NULL,
  ...
  if (exists("activity_log")) {
    ...
    assign(paste0("activity_log_",ts),activity_log)
    rm(activity_log, envir = .GlobalEnv)
  }
  make_log(refresh.data = TRUE)

  # Compare the local cache of tables with what is required -----------------
  local_table_status_check <- function(db=.GlobalEnv$db){
    localTables <- list.files(path = data.dir,
                              pattern = paste0("^",toupper(db),".*\\.rdata$"),
                              full.names = T,
                              ignore.case = TRUE,
                              recursive = FALSE)
    ...
    missingTables = sort(setdiff(paste0(toupper(db),'.',ds_all[[.GlobalEnv$db]]$tables),prefixed.localTables))
  ...
  ## data extraction ----------------------------------------------------
  oracle_activity<-function(tables = NULL,
                            oracle_cxn,
                            usepkg,
                            theschema,
                            prefix,
    ...
    if (action=='verify_access'){
      ...
      qry = paste0("select '1' from ",theschema,".",gsub(paste0(prefix,"."),"",tables)," WHERE ROWNUM<=1")
      ...
      if (tables %in% names(ds_all[[.GlobalEnv$db]]$table_err_roracle)){
        ...
        badvalues = paste(unlist(gsub("(.*)","\\1",this$badvalues)),sep="",collapse=",")
        add.where = paste0(this$field," NOT IN (",badvalues,")")
        cat(paste0("\n\tSkipping records ",theschema,".",tables,".",this$field," IN (",badvalues,")
                   \tThis\\These are from ",this$comments,"
                   \tIf this is critical, use RODBC instead of ROracle\n"))
      }
      table_naked = gsub(paste0(prefix,"."),"",tables)
      qry = paste0("SELECT * from ", theschema, ".",table_naked," WHERE ", add.where)
  ...
  try_extract<-function(usepkg, tables){
    ...
    if (!is.list(oracle_cxn)){
      ...
      if (oracle_cxn==-1)stop("Please check your credentials")
    }

    verified = sapply(tables, oracle_activity, oracle_cxn[[2]],oracle_cxn[[1]], ds_all[[.GlobalEnv$db]]$schema, toupper(db), 'verify_access')
    if (!all(verified))stop(
      "You do not have access to all of the required tables.
  ...
  # load the data -------------------------------------------------------
  try_load<-function(tables){
    loadit <- function(x){
      data(list=x, envir = .GlobalEnv)
      cat(paste0("\nLoaded ",x,"... "))
      fileAge=file.info(file.path(getwd(),"data",paste0(x,".RData")))$mtime
