...
#' @export
get_fleet_pre<-function(fn.oracle.username = "_none_", 
  ...
  Mar.datawrangling::get_data_custom('marfissci', tables = "MON_DOCS", data.dir = data.dir, quiet=T,env = fleetEnv, fn.oracle.username = fn.oracle.username, fn.oracle.password = fn.oracle.password, fn.oracle.dsn = fn.oracle.dsn, usepkg = usepkg) 
    fleetEnv$MON_DOCS = unique(fleetEnv$MON_DOCS[, names(fleetEnv$MON_DOCS) %in% c("MON_DOC_ID", 
                                                                           "MON_DOC_DEFN_ID",
                                                                           "VR_NUMBER")] )
  ...
  Mar.datawrangling::get_data_custom('marfissci', tables = "V_LICENCE_HISTORY", data.dir = data.dir, quiet=T,env = fleetEnv, fn.oracle.username = fn.oracle.username, fn.oracle.password = fn.oracle.password, fn.oracle.dsn = fn.oracle.dsn, usepkg = usepkg)  
    ...
    fleetEnv$V_LICENCE_HISTORY = unique(fleetEnv$V_LICENCE_HISTORY[, names(fleetEnv$V_LICENCE_HISTORY) %in% c("START_DATE_TIME", 
                                                                                                        ...
                                                                                                        "LICENCE_ID",
                                                                                                        "SPECIES_CODE")])
    fleetEnv$V_LICENCE_HISTORY$START_DATE_HIS = lubridate::floor_date(fleetEnv$V_LICENCE_HISTORY$START_DATE_TIME,unit = "day")
    fleetEnv$V_LICENCE_HISTORY$END_DATE_HIS = lubridate::floor_date(fleetEnv$V_LICENCE_HISTORY$END_DATE_TIME,unit = "day")
    fleetEnv$V_LICENCE_HISTORY$START_DATE_TIME <- fleetEnv$V_LICENCE_HISTORY$END_DATE_TIME <- NULL
  Mar.datawrangling::get_data_custom('marfissci', tables = "PRO_SPC_INFO", data.dir = data.dir, quiet=T,env = fleetEnv, fn.oracle.username = fn.oracle.username, fn.oracle.password = fn.oracle.password, fn.oracle.dsn = fn.oracle.dsn, usepkg = usepkg)  
    fleetEnv$PRO_SPC_INFO = unique(fleetEnv$PRO_SPC_INFO[, names(fleetEnv$PRO_SPC_INFO) %in% c("VR_NUMBER_FISHING", 
                                                                                          "VR_NUMBER_LANDING",
                                                                                          "GEAR_CODE",
                                                                                         "LICENCE_ID",
                                                                                         "MON_DOC_ID",
                                                                                         "DATE_FISHED",
                                                                                         "LANDED_DATE")])
    fleetEnv$PRO_SPC_INFO$DATE_FISHED = lubridate::floor_date(fleetEnv$PRO_SPC_INFO$DATE_FISHED,unit = "day")
    fleetEnv$PRO_SPC_INFO$LANDED_DATE = lubridate::floor_date(fleetEnv$PRO_SPC_INFO$LANDED_DATE,unit = "day")

# Apply Filters -----------------------------------------------------------
  fleetEnv$V_LICENCE_HISTORY = fleetEnv$V_LICENCE_HISTORY[fleetEnv$V_LICENCE_HISTORY$START_DATE_HIS != fleetEnv$V_LICENCE_HISTORY$END_DATE_HIS 
                                                          & fleetEnv$V_LICENCE_HISTORY$START_DATE_HIS< dateEnd
                                                          & fleetEnv$V_LICENCE_HISTORY$END_DATE_HIS> dateStart,]

  fleetEnv$PRO_SPC_INFO = fleetEnv$PRO_SPC_INFO[(fleetEnv$PRO_SPC_INFO$DATE_FISHED < dateEnd | fleetEnv$PRO_SPC_INFO$LANDED_DATE < dateEnd)
    ...
    if (!is.null(sectors)) {
      ...
      fleetEnv$MON_DOCS = fleetEnv$MON_DOCS[fleetEnv$MON_DOCS$SECTOR_ID  %in% sectors,]
    }
  ...
  res = merge(fleetEnv$PRO_SPC_INFO, fleetEnv$V_LICENCE_HISTORY)
